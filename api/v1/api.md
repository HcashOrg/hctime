# hctime API Specification

## V1

This document describes the REST API provided by a `hctimed` server.  This API allows users to create and upload hashes which are periodically submitted to the HcashOrg blockchain.  It also provides the ability to to confirm the addition of the hash to a timestamped collection along with showing and validating their inclusion in the HcashOrg blockchain.
...
Upload one or more digests to the time server.  The server will then add these digests to a collection and eventually to a transaction that goes in a HcashOrg block.  This method returns immediately with the collection the digest has been added to.  You must use the verify call to find out when it has been anchored to a block (which is done in batches at a set time interval that is not related to the api calls).
