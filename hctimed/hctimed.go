...
import (
	...
	"github.com/HcashOrg/hctime/hctimed/backend"
	"github.com/HcashOrg/hctime/hctimed/backend/filesystem"
	"github.com/HcashOrg/hctime/util"
...
// HctimeStore application context.
type HctimeStore struct {
...
func (d *HctimeStore) sendToBackend(w http.ResponseWriter, method, route, contentType, remoteAddr string, body *bytes.Reader) {
...
func (d *HctimeStore) proxyStatus(w http.ResponseWriter, r *http.Request) {
...
func (d *HctimeStore) proxyTimestamp(w http.ResponseWriter, r *http.Request) {
	b, err := ioutil.ReadAll(r.Body)
	r.Body.Close()
	if err != nil {
		util.RespondWithError(w, http.StatusBadRequest,
...
func (d *HctimeStore) proxyVerify(w http.ResponseWriter, r *http.Request) {
	b, err := ioutil.ReadAll(r.Body)
	r.Body.Close()
	if err != nil {
		util.RespondWithError(w, http.StatusBadRequest,
...
func (d *HctimeStore) status(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func (d *HctimeStore) timestamp(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func (d *HctimeStore) verify(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
...
func _main() error {
	...
	if !fileExists(loadedCfg.HTTPSKey) &&
		...
		err := util.GenCertPair("hctimed", loadedCfg.HTTPSCert,
			loadedCfg.HTTPSKey)
		if err != nil {
